# -*- coding: utf-8 -*-
"""
Created on Fri Aug 25 23:56:07 2023
...
control_variable = 'GDP per capita (constant 2015 US$)'
independent_variables = df_crisis2.columns.difference([dependent_variable, control_variable, 'Country', 'Crisis', 'Start', 'End', 'length_db', 'Mean_diff'])
...
pdf_filename = 'bivariate_regression_results_GDPcumulative_2Y_control.pdf'
...
with PdfPages(pdf_filename) as pdf:
    for ind_var in independent_variables:
        ...
        subset_df = df_crisis2[[dependent_variable, ind_var, control_variable]].dropna()
        
        # Apply natural logarithm to the control variable
        subset_df['ln_' + control_variable] = np.log(subset_df[control_variable])
        
        X = sm.add_constant(subset_df[[ind_var, 'ln_' + control_variable]])  # Add the intercept
        ...
        if ind_var in variables_to_log:
            ...
            X = sm.add_constant(subset_df[['ln_' + ind_var, 'ln_' + control_variable]])
...
print('Regression results saved to bivariate_regression_results_GDPcumulative_2Y_control.pdf')
